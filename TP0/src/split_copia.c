...
void copiar_palabra(const char* string_origen,char* string_destino,int posicion_copia,int cant_caracteres)
{
    strncpy(string_destino,string_origen + posicion_copia,cant_caracteres);
...
char** split(const char* string,char separador)
{
    ...
    char** vector_strings = malloc((contador_palabras + 1) * sizeof(char*));
    ...
    while(string[caracter] != CARACTER_NULO){
        if(string[caracter] == separador){
            ...
            vector_strings[posicion] = malloc((longitud_palabra + 1) * sizeof(char));
            if(vector_strings[posicion] == NULL){
                liberar_memoria(vector_strings,posicion);
                return NULL;
            }
            copiar_palabra(string,vector_strings[posicion],inicio_palabra,longitud_palabra);
            posicion++;
